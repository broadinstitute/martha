...
docker run \
    ...
    "gcloud config set project ${DEPLOY_PROJECT_NAME} &&
      ...
      cd ${MARTHA_PATH} &&
      gcloud beta functions deploy martha_v2 --trigger-http --source=. --runtime nodejs16 \\
        --allow-unauthenticated --project ${DEPLOY_PROJECT_NAME} --set-env-vars ENV=${DEPLOY_ENV} &&
      gcloud beta functions deploy martha_v3 --trigger-http --source=. --runtime nodejs16 \\
        --allow-unauthenticated --project ${DEPLOY_PROJECT_NAME} --set-env-vars ENV=${DEPLOY_ENV} &&
      gcloud beta functions deploy fileSummaryV1 --trigger-http --source=. --runtime nodejs16 \\
        --allow-unauthenticated --project ${DEPLOY_PROJECT_NAME} --set-env-vars ENV=${DEPLOY_ENV} &&
